...
public class ListaCircularDuplamente extends ListaDuplamente
{
	...
	public ListaCircularDuplamente merge(ListaCircularDuplamente lista2) {
		ListaCircularDuplamente l3 = new ListaCircularDuplamente();
		Elo p = prim.prox;
		Elo q = lista2.prim.prox;
		
		while((p != prim) && (q != lista2.prim)) {
			l3.insere(p.ant.dado);
			l3.insere(q.ant.dado);
			p = p.prox;
			q = q.prox;
			if((p == prim) && (q == lista2.prim)) {
				l3.insere(p.ant.dado);
				l3.insere(q.ant.dado);
			}
		}
