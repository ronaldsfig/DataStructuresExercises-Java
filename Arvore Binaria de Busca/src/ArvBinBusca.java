...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor> 
{
	...
	public class No
	{
		public Chave chave; /* Chave usada nas compara��es. */
    ...
    */
    ...
    public static ArvBinBusca<Integer,Integer> constroiArvore(int[] vetor) {
    	if(vetor == null) {
    		throw new IllegalArgumentException("A chave fornecida � null!");
    	}
    ...
    public boolean eSimilar(ArvBinBusca<Chave, Valor> arvore) {
    	return eSimilar(raiz, arvore.raiz);
    };
    
    private boolean eSimilar(No x, No y) {
    	if(x == null && y == null) {
    		return true;
    	}else if((x != null) && (y != null)) {
        	if(eSimilar(x.esq, y.esq) && eSimilar(x.dir, y.dir)){
        		return true;
        	}
    	}
    ...
    public boolean removeForaIntervalo(Chave chaveMin, Chave chaveMax) {
    	if((chaveMin.compareTo(chaveMax) > 0) || (chaveMax.compareTo(chaveMin) < 0)) {
    		throw new IllegalArgumentException("Chaves inválidas!");
    	}
    	
    	removeu = false;
    	removeForaIntervaloEsq(raiz, null, chaveMin, chaveMax);
    	removeForaIntervaloDir(raiz, null, chaveMin, chaveMax);
    	return removeu;
    };
    
    private No removeForaIntervaloEsq(No x, No ant, Chave min, Chave max) {
    	
    	if(x == null)
    		return null;
    	
    	int cmpEsq = x.chave.compareTo(min);
    	int cmpDir = x.chave.compareTo(max);
    	...
    	if(cmpEsq > 0){
    		...
    		x.esq = removeForaIntervaloEsq(x.esq, x, min, max);
    	}else if (cmpEsq < 0){ 
    		if(ant == null && x.dir == null) {
    			raiz = null;
    			removeu = true;
    			return raiz;
    		}else if(ant == null && x.dir != null) {
    			raiz = x.dir;
    			removeu = true;
    			return removeForaIntervaloEsq(raiz, null, min, max);
    		}
    		ant.esq = x.dir;
    		removeu = true;
    		if(ant.esq != null) {
    			return removeForaIntervaloEsq(ant.esq, ant, min, max);
    		}
    		return null;
    	}
    	
    	if(x.esq != null) {
    		if(x.esq.chave.compareTo(min) < 0) {
        		x.esq = null;
        		removeu = true;
        	}
    	}
    ...
    private No removeForaIntervaloDir(No x, No ant, Chave min, Chave max) {
    	...
    	if(cmpDir < 0){
    		...
    		x.dir = removeForaIntervaloDir(x.dir, x, min, max);
    	}else if (cmpDir > 0){ 
    		if(ant == null && x.dir == null) {
    			raiz = null;
    			removeu = true;
    			...
    			raiz = x.dir;
    			removeu = true;
    			return removeForaIntervaloDir(raiz, null, min, max);
    		}
    		ant.dir = x.esq;
    		removeu = true;
    		if(ant.dir != null) {
    			return removeForaIntervaloDir(ant.dir, ant, min, max);
    		}
    		return null;
    	}
    	
    	if(x.dir != null) {
        	if(x.dir.chave.compareTo(max) > 0) {
        		x.dir = null;
        		removeu = true;
        	}
    	}
    ...
    public No obtemAncestralComum(No primeiroNo, No segundoNo) {
    	...
    	if(cmp > 0) {
    		return obtemAncestralComum(raiz, segundoNo, primeiroNo);
    	}else if(cmp < 0) {
    ...
    private No obtemAncestralComum(No x, No min, No max) {
    	
    	if(x == null) {
    		return null;
    	}
    	
    	int cmpMin = x.chave.compareTo(min.chave);
    	int cmpMax = x.chave.compareTo(max.chave);
    	
    	if(x.esq.chave.compareTo(min.chave) == 0 || x.dir.chave.compareTo(max.chave) == 0 
    		|| x.esq.chave.compareTo(max.chave) == 0 || x.dir.chave.compareTo(min.chave) == 0) {
    		return x;
	    }
	    	
	    if(cmpMax > 0) {
	    	return obtemAncestralComum(x.esq, min, max);
	    }else if(cmpMin < 0) {
	    	return obtemAncestralComum(x.dir, min, max);
	    }
	    	
	    // após achar o ancestral, verifica se os nós estão dentro da árvore.
	    if(get(x, min.chave) != null && get(x, max.chave) != null) {
	    	return x;	
	    }
    ...
    private boolean eBalanceada(No x) {
    	int altEsq;
    	int altDir;
    	...
    	int sub = altEsq - altDir;
    	
    	if(sub == 0) {
    		return true;
    	}else if (sub == 1 || sub == -1) {
    		if(eBalanceada(x.esq) == true && eBalanceada(x.dir) == true) {
    			return true;
    		}
    	}
