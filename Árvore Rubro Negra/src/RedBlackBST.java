...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    ***************************************************************************/
    ...
    public Value get(Key key) {
        ...
        if(lastSearchedNode != null) {
            if(lastSearchedNode.key == key) {
            	return lastSearchedNode.val;
            }
        }
        return get(root, key);
    }
    ...
    */
	...
	private void mostra(Node x)
	{
		...
		char color = isRed(x) ? 'V' : 'P';
		System.out.print("<" + x.key + ":" + color +  ">");
    ...
    public int alturaNegra(Key key) {
    	Node node = getNode(key);
    	
    	if(node != null) {
    		return alturaNegra(node);
    	}
    	return Integer.MIN_VALUE;
    }
    
    private int alturaNegra(Node node) {
    	if(isRed(node)) {
    		return alturaNegra(node, 0);
    	}else {
    		int height = alturaNegra(node, 0);
    		if(height != 0) {
    			return --height;
    		}
    		return height;
    ...
    private int alturaNegra(Node node, int count) {
    	if(node == null) {
    		return count;
    	}
    	...
    	return alturaNegra(node.left, count);
    }
